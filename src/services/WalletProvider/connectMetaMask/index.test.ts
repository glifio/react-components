...
describe('connectMetaMask', () => {
  ...
  test('it returns an existing metamask provider if one is passed', async () => {
    ...
    const provider = await connectMetaMask(dispatchSpy, null, CoinType.TEST, '')
    jest.clearAllMocks()
    // @ts-ignore
    provider.wallet.mock = true
    const reusedProvider = await connectMetaMask(
      dispatchSpy,
      provider.wallet,
      CoinType.TEST,
      ''
  ...
  describe('error handling', () => {
    ...
    test('it handles metamask not installed errs', async () => {
      ...
      await connectMetaMask(dispatchSpy, null, CoinType.TEST, '')
    ...
    test('it handles metamask locked errs', async () => {
      jest
        .spyOn(require('../metamaskUtils'), 'metaMaskEnable')
        .mockImplementation(() => {
